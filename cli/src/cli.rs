...
impl Default for Args {
    fn default() -> Self {
        Self {
            project_name: Some("node-app".to_string()),
            manager: Some(PackageManager::Npm),
            template: Some(Template::Fastify),
            skip: false,
        }
    }
}
...
{YELLOW}ARGS:{RESET}
  {GREEN}<PROJECTNAME>{RESET}                 Specify project name which is used for the directory, package.json

{YELLOW}OPTIONS:{RESET}
  {GREEN}-m{RESET}, {GREEN}--manager <MANAGER>{RESET}       Specify preferred package manager [{managers}]
  {GREEN}-t{RESET}, {GREEN}--template <TEMPLATE>{RESET}     Specify the template to use [{fragments}]
